...
| `phone`                    | string  | No       | Australian mobile number (04/05 only)      |
...
All values below are matched case-insensitively, and `state` also accepts the full name (e.g. `New South Wales`). Responses return the canonical form.
