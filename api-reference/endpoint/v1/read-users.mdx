...
<Warning>
  There is no `external_user_id`, `partner_user_id` or any other partner-side identifier in this response, and no such field is stored by Parchment. `data` is a flat array of objects that hold exactly `user_id` and `email`. Map `user_id` to your own user record when you create the user.
</Warning>

<Note>
  The response is a flat array, not an object. There is no `data.users` wrapper and no `pagination` object. The endpoint takes no `limit` or `lastKey` query parameter — it returns every user in the organization.
</Note>
...
5. **Limited Data**: Only `user_id` and `email` are returned for security and performance reasons. No partner-side identifier is returned.
6. **Detailed Information**: Use the read-user endpoint (`GET /users/{user_id}`) to get full user details
7. **Pagination**: This endpoint returns all users in the organization. There is no pagination, and `limit` / `lastKey` query parameters are ignored.
