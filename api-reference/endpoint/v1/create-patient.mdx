...
```json
{
  ...
  "error": {
    ...
    "validation": [
      ...
      {
        "field": "phone",
        "message": "Phone number must be a valid Australian mobile number (e.g., 0412345678, +61412345678)",
        "code": "INVALID_FORMAT"
      },
      {
        "field": "postcode",
...
The `partner_id` field is **required** in the request body. It is your Parchment-assigned partner identifier (lowercase slug, e.g. `tacklit`).

- Omitted → 422 `"Partner ID is required"`
- Not registered with Parchment → 422 `"Invalid partner_id. Contact Parchment to register your partner ID."`
- Parchment registers your identifier during onboarding, per environment (sandbox and production) — contact Parchment before go-live

### Code List Casing

Code lists are matched case-insensitively — `sex`, `state`, `ihi_status`, `ihi_record_status` and `partner_id`. `nsw`, `Nsw` and `NSW` are the same value.

- `state` also accepts the full name, e.g. `New South Wales`
- `email` is stored lower case; `dva_file_number` and `entitlement_number` are stored upper case with separators removed
- Responses always return the canonical form, e.g. `NSW`

### Phone Number Format

The `phone` field accepts Australian **mobile** numbers only (04 / 05 ranges). Mobile numbers can be used for IHI lookup against the Healthcare Identifiers Service; landlines cannot, and are rejected.

**✅ Valid Examples:**

- `0412345678` - Mobile number (domestic format)
- `61412345678` - International mobile (without + prefix)
- `+61412345678` - International mobile (with + prefix)

**❌ Invalid Examples:**

- `0212345678` - Landline (only mobiles are accepted)
