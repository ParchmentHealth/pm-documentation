...
The user holds the `provider` role, but `provider_details` is `null`. **This user cannot create prescriptions.** See "Checking a prescriber is complete" below.
...
## Checking a prescriber is complete

Use this endpoint to confirm the result of a Create User or Update User call that included provider fields.

| `provider_details` | Meaning                                                                                          | What to do                                                                                    |
| ------------------ | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| Object             | The provider record exists. The HPI-I status is not returned.                                       | Nothing.                                                                                       |
| `null`             | The user has a provider role but **no provider record**. The user cannot create prescriptions.     | Call Update User with the corrected `provider_details`, then read the user again. |
| Absent             | The user holds no provider role. This is normal for `admin`, `receptionist` and `rx_reader` users. | Nothing.                                                                                       |

<Warning>
  A `USER_CREATED_PROVIDER_NOT_FOUND` or `USER_CREATED_PROVIDER_ALREADY_EXISTS` result from Create User always leaves `provider_details` as `null`. Read the user after any partial creation to confirm the repair worked.
</Warning>

<Note>
  There is no `external_user_id` or `partner_user_id` field. Parchment stores no partner-side identifier for a user — map `user_id` to your own record.
</Note>
...
| `provider_details` | object \| null | Provider-specific details. Present only if the user has the "provider" or "rx_queue_manager" role. `null` means the user has that role but no provider record |
...
5. **Provider Data Availability**: `provider_details` is `null` when the user holds a provider role but has no provider record. That user cannot create prescriptions — see "Checking a prescriber is complete" above
