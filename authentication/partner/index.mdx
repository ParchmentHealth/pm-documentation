...
<Note>
**Cache your access token.** Tokens are valid for **6 hours**. Request one, hold it in memory, and reuse it for every call until it expires. Request a new token only on expiry or on a `401`.

Do not request a token per API call. That doubles your request volume, adds a round trip to every call, and is the most common integration mistake we see.
</Note>
...
- Valid for 6 hours — read `expires_in` in the token response for the exact lifetime
