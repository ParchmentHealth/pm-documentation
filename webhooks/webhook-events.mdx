...
Generate the SSO token with the reserved values on the `redirect_path`:
...
curl --location 'https://api.sandbox.parchmenthealth.io/external/v1/sso' \
  ...
  --data '{
    "redirect_path": "/embed/patients/{patient_id}/prescriptions?reserved_1=test1&reserved_2=test2&reserved_3=test3"
  }'
```

When the user completes an action (e.g. creating a prescription), those values are
echoed on the webhook `metadata`:
...
```json
{
  ...
  "data": {
    ...
    "partner_patient_id": "UNKNOWN_PARTNER",
