...
|-------|------|-------------|-------------|
| `reserved_1` | string \| null | ≤ 30 chars | Partner pass-through value (echoed from the SSO request). `null` if not supplied. |
| `reserved_2` | string \| null | ≤ 30 chars | Partner pass-through value (echoed from the SSO request). `null` if not supplied. |
| `reserved_3` | string \| null | ≤ 30 chars | Partner pass-through value (echoed from the SSO request). `null` if not supplied. |

**How they are populated:** append `reserved_1` / `reserved_2` / `reserved_3` as
query parameters on the `redirect_path` when generating an SSO token (see
[Embedded iframe → Reserved parameters](/sso/embedded-iframe#reserved-parameters)).
...
A telehealth partner tying a prescription back to their own booking, consultation,
and acquisition channel generates the SSO token with those values on the
`redirect_path`:

```bash
curl --location 'https://api.sandbox.parchmenthealth.io/external/v1/sso' \
  --header 'x-organization-id: {organization_id}' \
  --header 'x-organization-secret: {organization_secret}' \
  --header 'x-partner-id: {partner_id}' \
  --header 'x-partner-secret: {partner_secret}' \
  --header 'x-user-id: {user_id}' \
  --header 'Content-Type: application/json' \
  --data '{
    "redirect_path": "/embed/patients/{patient_id}/prescriptions?reserved_1=BK-2026-4471&reserved_2=consult_88213&reserved_3=app-ios"
  }'
```

When the clinician completes the action (e.g. creating a prescription), those values
are echoed on the webhook `metadata` so the partner can reconcile the event against
their booking system:

```json
{
  "event_type": "prescription.created",
  "event_id": "evt_a2d36605a5ef6325206ee6bbaa38793e",
  "timestamp": "2026-07-05T00:24:13.278Z",
  "partner_id": "demo",
  "organization_id": "8d05a1af-6a6e-44e6-953e-6e1a0bee54ab",
  "data": {
    "patient_id": "57a8378e-2172-45c1-9329-cb800055eb0a",
    "partner_patient_id": "1523402100149593750",
    "user_id": "89be7478-10d1-702e-19d7-0346ac113984",
    "scid": "202VYFQQPM7FTP1Q86"
  },
  "metadata": {
...
interface PrescriptionCreatedEvent {
  ...
  metadata: {
    reserved_1: string | null; // Partner pass-through from SSO request (null if not supplied)
    reserved_2: string | null; // Partner pass-through from SSO request (null if not supplied)
    reserved_3: string | null; // Partner pass-through from SSO request (null if not supplied)
  };
}
```
