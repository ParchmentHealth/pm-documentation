...
<Note>
  **Prerequisite:** each organization must first call [`GET /v1/organizations/{organization_id}/validate`](/api-reference/endpoint/v1/validate-integration). Until `/validate` succeeds for an organization, no webhooks are sent for it.
</Note>
...
<Step title="Validate the organization">

Call [`GET /v1/organizations/{organization_id}/validate`](/api-reference/endpoint/v1/validate-integration) once per organization you connect. This is a prerequisite — it flags the organization as integrated with your platform, and Parchment only emits webhooks for organizations that have been validated.

</Step>

<Step title="Provide your webhook endpoint">

Contact Parchment support at [developer@parchment.health](mailto:developer@parchment.health) with:
...
Once retries stop, the event is **not re-sent automatically**. Failures are logged on Parchment's side; contact [developer@parchment.health](mailto:developer@parchment.health) if you need a failed event replayed.
